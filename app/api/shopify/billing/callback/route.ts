...
export async function GET(req: NextRequest) {
  ...
  // Accept ACCEPTED, ACTIVE, or PENDING (test charges on dev stores auto-activate)
  const validStatuses = ['ACCEPTED', 'ACTIVE', 'PENDING']
  if (!subscription || !validStatuses.includes(subscription.status)) {
    console.error('Subscription status invalid:', subscription?.status, 'chargeId:', chargeId, 'verifyJson:', JSON.stringify(verifyJson))
    return NextResponse.redirect(`${APP_URL}/dashboard/billing?error=charge_declined`)
  }

  // Activate the subscription (only if not already ACTIVE)
