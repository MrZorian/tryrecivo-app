        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          {plans.map(plan => (
            <div key={plan.name} className={`bg-white rounded-xl p-6 border shadow-sm relative ${plan.popular ? 'border-2' : 'border-gray-100'}`} style={plan.popular ? {borderColor:'#1a2f5e'} : {}}>
              {plan.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-bold text-white" style={{background:'#1a2f5e'}}>MOST POPULAR</div>}
