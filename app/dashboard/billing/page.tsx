...
const plans = [
  {
    id: 'free', name: 'Free', price: 0, emails: '500',
    features: ['1 store', 'Basic receipt style', 'Brand color customization', 'Full order breakdown'],
  ...
  {
    id: 'starter', name: 'Starter', price: 19, emails: '5,000', popular: true,
    features: ['1 store', 'Basic, Modern & Classic styles', 'Custom thank-you message', 'Return policy & disclaimer', 'Social media links in footer', 'Remove Recivo branding'],
    locked: ['Luxury style'],
  },
  {
    id: 'growth', name: 'Growth', price: 49, emails: '20,000',
    features: ['3 stores', 'All 4 receipt styles incl. Luxury', 'Everything in Starter', 'Custom sending domain', 'Analytics dashboard', 'Priority support'],
    locked: [],
  },
  {
    id: 'pro', name: 'Pro', price: 99, emails: 'Unlimited',
    features: ['Unlimited stores', 'All receipt styles', 'Everything in Growth', 'White-label (no Recivo branding)', 'API access', 'Dedicated support'],
    locked: [],
  },
]
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        {upgraded && (
          <div className="mb-6 bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600">Your new limits are active immediately.{source === 'stripe' && ' Billed via credit card.'}</p>
            </div>
          </div>
        )}
        ...
        {error && (
          <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 text-sm text-red-700">
            {error === 'charge_declined' ? '⚠️ Upgrade was cancelled. No charge was made.' : `⚠️ Something went wrong (${error}). Please try again.`}
          </div>
        )}
        ...
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-500">{emailsUsed.toLocaleString()} / {emailsLimit === 999999 ? '∞' : emailsLimit.toLocaleString()} emails</span>
          </div>
        ...
        {payMethod === 'shopify' && <p className="text-center text-xs text-gray-400 mb-6">Charged through your Shopify account. Manage in your Shopify admin.</p>}
        {payMethod === 'card' && <p className="text-center text-xs text-gray-400 mb-6">Secure checkout via Stripe. No Shopify store required.</p>}
            ...
            return (
              ...
              >
                ...
                <ul className="space-y-2 mb-6 flex-1">
                  {plan.features.map(f => <li key={f} className="flex items-start gap-2 text-sm text-gray-700"><span style={{ color: '#00bfa5' }}>✓</span> {f}</li>)}
                  {plan.locked.map(f => <li key={f} className="flex items-start gap-2 text-sm text-gray-300"><span>✗</span> {f}</li>)}
                </ul>
                {isCurrent ? (
                  ...
                  >
                    {upgrading === plan.id ? 'Redirecting...' : isHigher ? `Upgrade ${payMethod === 'card' ? '💳' : '🛍'}` : 'Downgrade'}
                  </button>
                )}
              </div>
            )
        ...
        <p className="text-center text-xs text-gray-400 mt-8">
          {payMethod === 'shopify' ? 'Billed via Shopify. Cancel anytime from your Shopify admin.' : 'Billed via Stripe. Secure card processing. Cancel anytime.'}
        </p>
      </div>
    </div>
  )
