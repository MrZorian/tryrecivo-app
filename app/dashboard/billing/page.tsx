...
function BillingContent() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Sync the plan against Shopify's real subscription state. This ensures the
      // UI reflects the chosen plan even if the merchant lands here straight from
      // a Shopify billing approval (managed pricing doesn't call our callback).
      try {
        const res = await fetch('/api/shopify/billing/status', { cache: 'no-store' })
        if (res.ok) {
          const data = await res.json()
          if (data?.plan) setCurrentPlan(data.plan)
          if (typeof data?.emails_limit === 'number') setEmailsLimit(data.emails_limit)
        }
      } catch { /* non-fatal: keep the stored plan */ }
    }
    load()
  }, [])
