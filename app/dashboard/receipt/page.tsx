...
const STYLES = [
  {
    id: 'basic', name: 'Basic', desc: 'Bold header with brand color', minPlan: 'free',
  ...
  {
    id: 'modern', name: 'Modern', desc: 'Clean serif typography', minPlan: 'starter',
    preview: () => (
  ...
  {
    id: 'classic', name: 'Classic', desc: 'Traditional table layout', minPlan: 'starter',
    preview: (color: string) => (
  ...
  {
    id: 'luxury', name: 'Luxury', desc: 'Dark elegant with gold accents', minPlan: 'growth',
    preview: () => (
...
const PLAN_BADGE: Record<string, string> = {
  free: '#94a3b8', starter: '#00bfa5', growth: '#f59e0b', pro: '#8b5cf6',
}
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-full lg:w-[460px] xl:w-[500px] overflow-y-auto flex-shrink-0 px-6 py-8 space-y-6 border-r border-gray-100">
            ...
            <div className="grid grid-cols-2 gap-3">
              {STYLES.map(s => {
                ...
                return (
                  <button
                    key={s.id}
                    onClick={() => { if (locked) return; setSettings({ ...settings, receipt_style: s.id }) }}
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea value={settings.thank_you_message} onChange={e => setSettings({ ...settings, thank_you_message: e.target.value })} rows={3} className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm text-gray-900 resize-none focus:outline-none focus:ring-2 focus:ring-[#00bfa5]" placeholder="Thank your customers..." />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Return policy</label>
                <textarea value={settings.return_policy} onChange={e => setSettings({ ...settings, return_policy: e.target.value })} rows={3} className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm text-gray-900 resize-none focus:outline-none focus:ring-2 focus:ring-[#00bfa5]" placeholder="e.g. Returns accepted within 30 days with receipt." />
              </div>
              <div>
                ...
                <textarea value={settings.disclaimer} onChange={e => setSettings({ ...settings, disclaimer: e.target.value })} rows={2} className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm text-gray-900 resize-none focus:outline-none focus:ring-2 focus:ring-[#00bfa5]" placeholder="Any legal disclaimer, tax notice, etc." />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {SOCIAL_PLATFORMS.map(platform => (
                <div key={platform.id} className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-white text-xs font-bold" style={{ background: platform.color }}>
            ...
            <div className="space-y-3">
              {[
                ...
                <label key={item.key} className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={(settings as any)[item.key]} onChange={e => setSettings({ ...settings, [item.key]: e.target.checked })} className="w-4 h-4 accent-[#00bfa5]" />
          ...
          <button onClick={handleSave} disabled={saving || !store} className="w-full py-3 rounded-lg text-white font-semibold transition-colors" style={{ background: saving ? '#94a3b8' : '#00bfa5' }}>
